...
import arenafacil.core.exceptions.mapper.ExceptionInfoHolder;
...
public class UtilRest {
    ...
    /**
     * Cria texto de erro com campos status, message (se status válido), description, codigoErro, codigoErroStr e causa (se existir)
     *
    ...
    public static String createErrorJson(ObjectNode jsonNodes, ExceptionInfoHolder exceptionInfoHolder, Throwable exception) {
        ...
        CodigoErro codigoErro = exceptionInfoHolder.codigoErro();

        jsonNodes.put("status", statusCode);
        jsonNodes.put("description", msg);
        jsonNodes.put("codigoErro", codigoErro.codigo);
        jsonNodes.put("codigoErroStr", codigoErro.name());
